...
def batch_transform(batch, transform):
    ...
    Code adapted from:     https://github.com/alexgkendall/SegNet-Tutorial/tree/master/CamVid.
...
def imshow_batch_2(images, predict_labels,gt_labels):
    """Displays two grids of images. The top grid displays ``images``
    and the bottom grid ``labels``
    ...
    # Make a grid with the images and labels and convert it to numpy
    images = torchvision.utils.make_grid(images).numpy()
    pred_labels = torchvision.utils.make_grid(predict_labels).numpy()
    gt_labels = torchvision.utils.make_grid(gt_labels).numpy()

    fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(15, 7))
    ax1.imshow(np.transpose(images, (1, 2, 0)))
    ax2.imshow(np.transpose(pred_labels, (1, 2, 0)))
    ax3.imshow(np.transpose(gt_labels, (1, 2, 0)))
    # plt.axis('off')
    # plt.savefig("taaaaest.png", bbox_inches='tight')
...
def save_checkpoint(model, optimizer, epoch, miou, args):
    ...
    if torch.cuda.device_count() > 1: 
        torch.save(model.module.state_dict(), model_path+'_Model')
    else:
        torch.save(model.state_dict(), model_path+'_Model')

def save_checkpoint_epoch(model, optimizer, epoch, miou, args):
    """Saves the model in a specified directory with a specified name.save
...
def load_checkpoint(model, optimizer, folder_dir, filename,args):
    """Saves the model in a specified directory with a specified name.save
    ...
    if args.checkpoint_test == 'none':
        # Load the stored model parameters to the model instance
        checkpoint = torch.load(model_path)
        print('model_path',model_path)
        model.load_state_dict(torch.load(model_path+'_Model'))
