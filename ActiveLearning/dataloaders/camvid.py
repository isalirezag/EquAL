...
class CamVid(data.Dataset):
    ...
    # if training on original camvid
    
    train_folder_al = 'AL_Iter_RGB'
    train_lbl_folder_al = 'AL_Iter_Ann'
    ...
    def __init__(self,
        ...
        if self.mode.lower() == 'train':
            ...
            self.test_labels = utils.get_files(
                ...
                extension_filter=self.img_extension)
        elif self.mode.lower() == 'train_al':
            # Get the training data and labels filepaths
            self.train_al_data = utils.get_files(
                os.path.join(root_dir, self.train_folder_al),
                extension_filter=self.img_extension)

            self.train_al_labels = utils.get_files(
                os.path.join(root_dir, self.train_lbl_folder_al),
                extension_filter=self.img_extension)
        else:
    ...
    def __getitem__(self, index):
        ...
        if self.mode.lower() == 'train':
            ...
            data_path, label_path = self.train_al_data[index], self.train_al_labels[
                index]
        else:
            raise RuntimeError("Unexpected dataset mode. "
                               "Supported modes are: train, val and test")
